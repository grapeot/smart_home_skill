...
def test_status_fast_subset_does_not_wait_for_wemo(mock_wemo, mock_rinnai, mock_garage, mock_hue):
...
@patch('api.status.rinnai_service.get_status', new_callable=AsyncMock)
@patch('api.status.wemo_service.get_all_status')
def test_status_wemo_subset_does_not_query_fast_devices(mock_wemo, mock_rinnai, mock_garage, mock_hue):
